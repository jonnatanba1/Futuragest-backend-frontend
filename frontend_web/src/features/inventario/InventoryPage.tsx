        ...
        <Card withBorder style={tableContainment}>
          ...
          <ScrollArea>
            <Table striped highlightOnHover miw={720}>
              <Table.Thead>
                <Table.Tr>
                  <Table.Th>Municipio / origen</Table.Th>
                  <Table.Th>SKU</Table.Th>
                  <Table.Th>Producto</Table.Th>
                  <Table.Th ta="right">Saldo base</Table.Th>
        ...
        <Card withBorder style={tableContainment}>
          ...
          <ScrollArea>
            <Table striped highlightOnHover miw={720}>
              <Table.Thead>
                <Table.Tr>
                  <Table.Th>Municipio / bodega</Table.Th>
                  <Table.Th>Producto</Table.Th>
      ...
      >
        <Stack>
          <Text size="sm" c="dimmed">
            Registra una compra o reposicion sin costo para una bodega central.
          </Text>
          <TextInput
            readOnly
          ...
          <Textarea
            label="Nota"
            description="Opcional: proveedor, remision u observacion."
            autosize
            minRows={3}
            maxRows={8}
...
function MasterDataPanel({ canAdmin }: { canAdmin: boolean }) {
  ...
  const automaticUnitFactor = unitProduct
    ? automaticFactorToBase(unitProduct.baseUnitCode, unitForm.values.unitCode)
    : null;
  ...
  return (
    <QueryBoundary queries={canAdmin ? [products, locations, balances] : [products, balances]}>
      <Stack gap="lg">
            ...
            <Table striped miw={680}>
              <Table.Thead>
                <Table.Tr>
                  <Table.Th>SKU</Table.Th>
                  <Table.Th>Producto</Table.Th>
                  <Table.Th>Unidades vigentes</Table.Th>
                  <Table.Th ta="right">Central</Table.Th>
                  <Table.Th ta="right">Municipios</Table.Th>
        ...
        {canAdmin && (
          <Modal
            opened={minimumOpened}
            onClose={() => setMinimumOpened(false)}
            title="Mínimos por municipio"
            zIndex={300}
          >
            <form
          ...
          <Stack>
              ...
              >
                ...
                <SimpleGrid cols={{ base: 1, sm: 2 }} mt="lg">
                  ...
                  >
                    ...
                    <Text size="xl" fw={800}>
                      {quantity(centralStock[detailProduct.id] ?? 0)}
                    </Text>
                  </Card>
                  <Card
                    radius="md"
                  ...
                  >
                    ...
                    <Text size="xl" fw={800}>
                      {quantity(municipalStock[detailProduct.id] ?? 0)}
                    </Text>
                  </Card>
                </SimpleGrid>
              </Card>
            ...
            <Card withBorder>
              <Group justify="space-between" mb="xs">
                <Group gap="xs">
                  <Text fw={600}>Entradas y salidas</Text>
                  {productMovements.isLoading && <Loader size="xs" />}
                </Group>
              ...
              {productMovements.hasNextPage && (
                <Button
                  mt="sm"
                  size="xs"
                  variant="default"
                  loading={productMovements.isFetchingNextPage}
                  onClick={() => void productMovements.fetchNextPage()}
                >
            ...
            <Card withBorder>
              <Group justify="space-between" mb="xs">
                <Group gap="xs">
                  <Text fw={600}>Envíos a municipios</Text>
                  {shipments.isLoading && <Loader size="xs" />}
                </Group>
                <Menu shadow="md" width={220} position="bottom-end">
                  <Menu.Target>
              ...
              <ScrollArea type="always" h={220} scrollbarSize={10}>
                <Table striped miw={560}>
                  <Table.Thead>
                    <Table.Tr>
                      <Table.Th>Fecha</Table.Th>
                      <Table.Th>Municipio destino</Table.Th>
                      <Table.Th ta="right">Enviado</Table.Th>
            ...
            {canAdmin && detailProduct && (
              <Group grow>
                <Button
                  onClick={() => {
                    setEntryProductId(detailProduct.id);
                    setEntryOpened(true);
                  }}
                >
                  Registrar ingreso
                ...
                <Button
                  ...
                  onClick={() =>
                    updateProduct.mutate(
                      { id: detailProduct.id, active: !detailProduct.active },
                      { onError: failure },
                    )
                  }
                >
        ...
        {canAdmin && entryOpened && (
          <StockEntryModal
            ...
            initialProductId={entryProductId ?? undefined}
          />
        )}
        <Modal
        ...
        <Modal
          opened={unitProductId !== null}
          onClose={() => setUnitProductId(null)}
          title="Agregar unidad"
          zIndex={300}
        >
          <form
            onSubmit={unitForm.onSubmit((values) => {
          ...
          >
            <Stack>
              ...
              <Button type="submit" loading={addUnit.isPending}>
                Agregar unidad
              </Button>
            </Stack>
          </form>
    ...
    dispatchShipment.mutate(
      {
        ...
        clientCommandId: stableInventoryCommandId(operationKey, {
          shipmentId: confirmAction.shipment.id,
        }),
      },
      {
        onSuccess: () => {
      ...
      <Stack>
        {canAdmin && (
          <Button
            ...
            w="fit-content"
          >
            Asignar productos a un municipio
          </Button>
        )}
        <Card withBorder style={tableContainment}>
          <Group justify="space-between" mb="md">
            <div>
              ...
              <Text size="sm" c="dimmed">
                Seleccioná un registro para ver el detalle completo.
              </Text>
            </div>
        ...
        <Modal
          ...
          title="Asignar productos a un municipio"
          size="lg"
        >
          <form
            onSubmit={form.onSubmit((values) =>
        ...
        >
          <Stack>
            ...
            <Button
              ...
              loading={returnShipment.isPending || resolveDiscrepancy.isPending}
      ...
      <Stack>
        <Card withBorder>
          <Group align="end">
            <Select
              searchable
              label="Ubicación para conteo ciego"
              value={locationId}
                        ...
                        {selected.status === 'OPEN' ? (
                          <TextInput
                            ...
                            onChange={(event) =>
                              setCounted((current) => ({
                                ...current,
                                [line.productId]: event.currentTarget.value,
                              }))
                            }
                          />
                        ) : (
        ...
        >
          <Stack>
            ...
            <Button
              ...
              loading={approveCount.isPending}
              onClick={approveSelectedCount}
            >
              Aprobar y ajustar
            </Button>
          </Stack>
...
function ReviewsPanel({ canReview }: { canReview: boolean }) {
  ...
  if (!canReview)
    return (
      <Alert color="blue">
        Tu rol puede consultar inventario, pero no resolver eventos retenidos.
      </Alert>
    );
  ...
  return (
    <Stack>
      ...
      <Modal
        opened={selected !== null}
        onClose={() => setSelected(null)}
        title="Resolver evento retenido"
      >
        <form
          onSubmit={form.onSubmit((values) => {
            if (!selected) return;
...
function MovementsPanel({ canAdmin }: { canAdmin: boolean }) {
  ...
  return (
    <QueryBoundary queries={[movements]}>
      <Stack>
        <Card withBorder style={tableContainment}>
          <ScrollArea>
            <Table striped miw={980}>
              <Table.Thead>
                <Table.Tr>
                  <Table.Th>Fecha</Table.Th>
                  <Table.Th>Ubicación</Table.Th>
  ...
  return (
    <Stack>
      ...
      <Card withBorder>
        <Stack>
          <FileInput
            label="Archivo CSV reconciliado"
            description="Columnas: locationCode, productSku, quantityBase"
            value={file}
            onChange={readFile}
            accept="text/csv,.csv"
            clearable
          ...
          {rows.length > 0 && (
            <Text size="sm">
              {rows.length} filas listas · SHA-256 {sourceHash.slice(0, 12)}…
            </Text>
          )}
          <Button
            disabled={!rows.length || !sourceHash}
    ...
    <Stack gap="lg">
      ...
      <Tabs defaultValue="overview" keepMounted={false}>
        ...
        <Tabs.Panel value="overview" pt="lg">
          <InventoryOverview
            canReconcile={!isPurchasing && canAdmin}
            canReview={!isPurchasing && canReview}
          />
        </Tabs.Panel>
        <Tabs.Panel value="catalog" pt="lg">
          <MasterDataPanel canAdmin={canAdmin} />
        </Tabs.Panel>
        <Tabs.Panel value="shipments" pt="lg">
          <ShipmentsPanel canAdmin={canAdmin} />
        </Tabs.Panel>
        {!isPurchasing && (
          <>
            <Tabs.Panel value="counts" pt="lg">
              <CountsPanel canApprove={canReview} />
            </Tabs.Panel>
            <Tabs.Panel value="reviews" pt="lg">
              <ReviewsPanel canReview={canReview} />
            </Tabs.Panel>
            <Tabs.Panel value="movements" pt="lg">
              <MovementsPanel canAdmin={canAdmin} />
            </Tabs.Panel>
            <Tabs.Panel value="import" pt="lg">
              <OpeningImportPanel canAdmin={canAdmin} />
            </Tabs.Panel>
          </>
        )}
      </Tabs>
